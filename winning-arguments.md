...
This discussion of the axiom of regularity, like our discussion of the axiom of extensionality, serves to illustrate that ZFC is not the only possible way of axiomatizing sets, and one can imagine conflicting conceptions of sets. For example, [New Foundations](https://en.wikipedia.org/wiki/New_Foundations) (so called) is a set theory in which the axiom of regularity is false, and which conflicts with ZFC on many points, but which (at least in weakened forms such as [NFU](https://en.wikipedia.org/wiki/New_Foundations#Models_of_NFU)) is logically consistent (if ZFC is logically consistent).
