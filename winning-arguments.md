...
In short, here is the conclusion I've arrived at. The sequent (**A1**,...,**An** entails **B**) is valid if and only if there is no possible set of precise meanings for the vocabulary in the sequent under which **A1**,...,**An** are true and **B** is false. In any conversational context which operates under the assumption that it ought to be possible in principle (as a coherence condition of the conversation that speakers intend to be true) to make the vocabulary being used completely precise, and where **A1**,...,**An** are held to be true, this allows one to infer that **B** is true, provided one does not find fault with the argument I have given. In this sense, the rules of classical logic are correct, again provided one does not find fault with the argument.
