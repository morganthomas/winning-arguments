...
The one-sample *t*-test is an example of a frequentist statistical method, or at least a method to which to a frequentist interpretation can be applied. It considers the set of all possible outcomes of the process of taking a random sample from the population of the same size as the actually available sample. Within this it measures the size of the set of such outcomes where the deviation of the sample mean from the expected population mean is at least the observed devation. This gives the probability of observing a deviation such as the observed deviation, assuming the actual population mean equals the expected mean (or in other words assuming the null hypothesis is true). 
